...
| Model Context Protocol (MCP) | `semcod/mcp` | MCP JSON-RPC 2.0 STDIO protocol | tool call requests (`data2dsl_compare`, `data2dsl_self_test`, `data2dsl_validate_envelope`, `data2dsl_simulate_healing`) | tool call results with text contents | MCP 2024-11-05 spec | `src/data2dsl_skill.py` (`handle_mcp_message`, `main_mcp`) | **IMPLEMENTED** | Native IDE MCP tool discovery and execution. |
| SUMD structured-document parsing | `semcod/sumd` / `data2dsl` | `SUMDAdapter` in `src/data2dsl_adapters.py` | SUMD-conformant Markdown tables and descriptors | normalized metric observations (`integer`, `float`, `percentage`, `string-set`) with SHA-256 evidence | SUMD markdown format | `src/data2dsl_adapters.py`, `tests/test_sumd_adapter.py` | **IMPLEMENTED** | Factual table and descriptor metric extraction. |
| Subactor Delegation Envelope Conformance | `wellmanifest/how-to-use-subactor` / `data2dsl` | `SubactorDelegationEnvelope`, `validate_delegation_envelope` in `src/data2dsl_subactor.py` | Text or JSON semantic envelopes (`ROLE`, `GOAL`, `SCOPE`, `ACCEPTANCE`, `AUTHORITY`, `LIMITS`, `REPORT`) | typed validated envelope or standard error codes (`COMM-ENVELOPE-001`, `COMM-ROLE-001`, `COMM-AUTH-001`) | Subactor normative standard | `src/data2dsl_subactor.py`, `tests/test_subactor_envelope.py` | **IMPLEMENTED** | Conformance validator and CLI `validate-envelope`. |
| Closed-Loop Self-Healing Simulation | `wellmanifest/how-to-use-subactor` / `data2dsl` | `simulate_self_healing_cycle` in `src/data2dsl_subactor.py`, CLI `simulate-healing` | query, left baseline, right observation | 5-stage verification (`DETECT` -> `PLAN` -> `EXECUTE` -> `VERIFY` -> `HEAL`) | self-healing closed loop | `src/data2dsl_subactor.py`, `tests/test_self_healing_e2e.py` | **IMPLEMENTED** | Reproducible end-to-end self-healing simulation with verified evidence digests. |
